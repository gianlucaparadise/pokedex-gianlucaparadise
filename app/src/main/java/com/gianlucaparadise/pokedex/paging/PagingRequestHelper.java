 ...
 *     com.gianlucaparadise.githubbrowser.util.PagingRequestHelper helper = new com.gianlucaparadise.githubbrowser.util.PagingRequestHelper(executor);
 *     // imaginary API service, using Retrofit
 *     MyApi api;
 *
 *     {@literal @}Override
 *     public void onItemAtFrontLoaded({@literal @}NonNull MyItem itemAtFront) {
 *         helper.runIfNotRunning(com.gianlucaparadise.githubbrowser.util.PagingRequestHelper.RequestType.BEFORE,
 ...
 *         helper.runIfNotRunning(com.gianlucaparadise.githubbrowser.util.PagingRequestHelper.RequestType.AFTER,
 ...
 * <pre>
 * MutableLiveData&lt;com.gianlucaparadise.githubbrowser.util.PagingRequestHelper.Status> combined = new MutableLiveData&lt;>();
 * helper.addListener(status -> {
 *     // merge multiple states per request type into one, or dispatch separately depending on
 *     // your application logic.
 *     if (status.hasRunning()) {
 *         combined.postValue(com.gianlucaparadise.githubbrowser.util.PagingRequestHelper.Status.RUNNING);
 *     } else if (status.hasError()) {
 *         // can also obtain the error via {@link StatusReport#getErrorFor(RequestType)}
 *         combined.postValue(com.gianlucaparadise.githubbrowser.util.PagingRequestHelper.Status.FAILED);
 *     } else {
 *         combined.postValue(com.gianlucaparadise.githubbrowser.util.PagingRequestHelper.Status.SUCCESS);
 *     }
 * });
 * </pre>
 */
...
public class PagingRequestHelper {
    ...
    /**
     * Creates a new com.gianlucaparadise.githubbrowser.util.PagingRequestHelper with the given {@link Executor} which is used to run
     * retry actions.
     *
